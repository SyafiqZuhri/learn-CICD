pipeline {
    // Kita pakai agent ANY (jalan di mesin Jenkins langsung, bukan di GKE)
    agent any
        ...
        stage('2. Build & Push Image (Pakai Kaniko via Docker)') {
            steps {
                ...
                withCredentials([file(credentialsId: 'gcp-credentials', variable: 'GCP_KEY')]) {
                    sh '''
                        // Login ke GCP pake file JSON
                        gcloud auth activate-service-account --key-file=$GCP_KEY

                        // Jalankan Kaniko di dalam Docker (biar build image tanpa perlu Docker daemon)
